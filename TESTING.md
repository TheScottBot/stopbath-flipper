...
|---|---|---|
| `tests/test_remote_input_model.c` | host, any `gcc` | `make test` |
| the same, under address and undefined behaviour sanitisers | Linux or WSL | `make test-sanitise` |
...
- The sanitiser build cannot run under the MinGW `gcc` on the Windows
  ...
  on the development machine; WSL on the same machine is how that is met.
