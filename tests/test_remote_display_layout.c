...
static void every_fixture_composes_within_bounds_with_a_header(RemoteTestReport* report) {
    ...
    for(int fixture_index = 0; fixture_index < remote_display_fixture_count(); fixture_index++) {
        ...
        const RemoteLayoutText* header = remote_display_layout_find_text(&layout, "StopBath");
        REMOTE_TEST_ASSERT(report, header != NULL, fixture->fixture_name);
...
static void an_error_is_an_inverted_band_at_the_bottom(RemoteTestReport* report) {
    ...
    assert_text_placed(report, &guest, (ExpectedText){REMOTE_DISPLAY_FIXTURE_ERROR_CODE, 64, 61, RemoteLayoutFontSecondary, RemoteLayoutAnchorCenter, true});
...
static void locked_inverts_the_header_on_every_screen(RemoteTestReport* report) {
    const char* locked_fixtures[] = {"presenting wifi locked", "ready locked"};
    for(int fixture_index = 0; fixture_index < 2; fixture_index++) {
        RemoteDisplayLayout layout = compose_named(report, locked_fixtures[fixture_index]);
        REMOTE_TEST_ASSERT(report, shape_present(&layout, RemoteLayoutShapeFilledBox, 0, 0, 128, REMOTE_LAYOUT_HEADER_HEIGHT), locked_fixtures[fixture_index]);
    ...
    RemoteDisplayLayout presenting = compose_named(report, "presenting wifi locked");
    REMOTE_TEST_ASSERT(report, presenting.qr_area_shown, "locking does not hide the code being presented");
...
int main(void) {
    static const RemoteTestCase test_cases[] = {
        ...
        {"locked inverts the header on every screen", locked_inverts_the_header_on_every_screen},
